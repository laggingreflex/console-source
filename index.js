...
let line = false || process.env.CONSOLE_SOURCE_LINE || 4;

const getFilename = () => {
  const stack = new Error("Dummy error used to find out file location").stack
    .split(/[\n]/g)
    .map((s) => s.trim())
...
const modifyConsoleMessage = (input) => {
  ...
  if (output.length > paddingLength) {
    ...
    padding =
      " " +
      Array(paddingLength - output.length)
        .fill(paddingFiller)
        .join("");
  }
  ...
  return (
    output +
    "\n" +
    Array(process.stdout.columns - filename.length)
      .fill(".")
      .join("") +
    filename
  );
...
let disable = () => {};
if (isEnabled) {
  disable = consoleInterceptor(
    (m, msg) => {
      if (
        global.consoleSource === false ||
        (global.consoleSource && global.consoleSource.disable)
      ) {
        disable();
        return;
      }
    ...
    (error, { console, disable, log, logError }) => {
      ...
      logError();
      console.warn(
        `Note: console-source disabled. This node process's global 'console' which was patched by 'console-source' has now been reverted back`,
      );
      log();
    },
  );
  if (!isSilent) {
    console.warn(
      `Note: console-source enabled. This node process's global 'console' is patched by 'console-source'`,
    );
  }
}
